...
export function connectPreviewDaemon(options: ClientOptions = {}): PreviewApi & SecretSetupApi & PreviewManagementApi & {
  ...
  return {
    ...
    configureBindings: (name, attemptId, changes, options, request) => call('configureBindings', { name, attemptId, changes, options }, request?.signal),
    ...
    start: (spec, options = {}) => call('start', { spec, expected: options.expected, sourceFile: options.sourceFile }),
    replace: (name, spec, options = {}) => call('replace', { name, spec, expected: options.expected, sourceFile: options.sourceFile }),
    list: () => call('list', {}),
