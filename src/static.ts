...
export async function startStatic(directory: string, spa: boolean): Promise<Resource> {
  const sockets = new Set<Socket>();
  let stopping: Promise<void> | undefined;
  let lost!: (error: Error) => void;
  const exited = new Promise<Error>((resolve) => { lost = resolve; });
  const server = http.createServer((request, response) => {
    void serve(directory, spa, request, response).catch(() => {
...
async function serve(directory: string, spa: boolean, request: http.IncomingMessage, response: http.ServerResponse): Promise<void> {
  ...
  try {
    ...
    if (!isWithin(directory, filename)) { error(403, 'Path is not available.'); return; }
  ...
  if (!isWithin(directory, filename) || path.relative(directory, filename).split(path.sep).some((part) => part.startsWith('.'))) { error(403, 'Path is not available.'); return; }
